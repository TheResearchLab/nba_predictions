...
def main():
    ...
    st.session_state.model = loads(bytes.fromhex(data['model_object_hex'][0]))


    url = "https://nba-prediction-api.onrender.com/upcoming_games"
    response = get(url)
    upcoming_games = read_json(response.json(),orient='records')
    ...
    def game_prediction(home_id,away_id):
        ...
        return outcomes[st.session_state.model.predict(array([*home_features,*away_features]).reshape(1,14))[0]]
